...
#include UE_INLINE_GENERATED_CPP_BY_NAME(RCAudioMixEffectsSubsystem)

class FSubsystemCollectionBase;

void URCAudioMixEffectsSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	Super::Initialize(Collection);
}

void URCAudioMixEffectsSubsystem::Deinitialize()
{
...
bool URCAudioMixEffectsSubsystem::ShouldCreateSubsystem(UObject* Outer) const
{
...
void URCAudioMixEffectsSubsystem::PostInitialize()
{
	if (const URCAudioSettings* RCAudioSettings = GetDefault<URCAudioSettings>())
	{
		if (UObject* ObjPath = RCAudioSettings->DefaultControlBusMix.TryLoad())
		{
			...
			{
				ensureMsgf(SoundControlBusMix, TEXT("Default Control Bus Mix reference missing from RC Audio Settings."));
			}
		}

		if (UObject* ObjPath = RCAudioSettings->LoadingScreenControlBusMix.TryLoad())
		{
			if (USoundControlBusMix* SoundControlBusMix = Cast<USoundControlBusMix>(ObjPath))
			{
			...
			{
				ensureMsgf(SoundControlBusMix, TEXT("Loading Screen Control Bus Mix reference missing from RC Audio Settings."));
			}
		}
		...
		if (UObject* ObjPath = RCAudioSettings->UserSettingsControlBusMix.TryLoad())
		{
			...
			{
				ensureMsgf(SoundControlBusMix, TEXT("User Control Bus Mix reference missing from RC Audio Settings."));
			}
		}
		...
		if (UObject* ObjPath = RCAudioSettings->OverallVolumeControlBus.TryLoad())
		{
			...
			{
				ensureMsgf(SoundControlBus, TEXT("Overall Control Bus reference missing from RC Audio Settings."));
			}
		}
		...
		if (UObject* ObjPath = RCAudioSettings->MusicVolumeControlBus.TryLoad())
		{
			...
			{
				ensureMsgf(SoundControlBus, TEXT("Music Control Bus reference missing from RC Audio Settings."));
			}
		}
		...
		if (UObject* ObjPath = RCAudioSettings->SoundFXVolumeControlBus.TryLoad())
		{
			...
			{
				ensureMsgf(SoundControlBus, TEXT("SoundFX Control Bus reference missing from RC Audio Settings."));
			}
		}
		...
		if (UObject* ObjPath = RCAudioSettings->DialogueVolumeControlBus.TryLoad())
		{
			...
			{
				ensureMsgf(SoundControlBus, TEXT("Dialogue Control Bus reference missing from RC Audio Settings."));
			}
		}
		...
		if (UObject* ObjPath = RCAudioSettings->VoiceChatVolumeControlBus.TryLoad())
		{
			...
			{
				ensureMsgf(SoundControlBus, TEXT("VoiceChat Control Bus reference missing from RC Audio Settings."));
			}
		}

		// Load HDR Submix Effect Chain
		for (const FRCSubmixEffectChainMap& SoftSubmixEffectChain : RCAudioSettings->HDRAudioSubmixEffectChain)
		{
			FRCAudioSubmixEffectsChain NewEffectChain;
		...
		for (const FRCSubmixEffectChainMap& SoftSubmixEffectChain : RCAudioSettings->LDRAudioSubmixEffectChain)
		{
			FRCAudioSubmixEffectsChain NewEffectChain;
...
void URCAudioMixEffectsSubsystem::OnWorldBeginPlay(UWorld& InWorld)
{
	if (const UWorld* World = InWorld.GetWorld())
	{
		...
		if (const URCSettingsLocal* RCSettingsLocal = GetDefault<URCSettingsLocal>())
		{
			...
			if (UserMix)
			{
				...
				{
					const FSoundControlBusMixStage OverallControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, OverallControlBus, RCSettingsLocal->GetOverallVolume());
					const FSoundControlBusMixStage MusicControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, MusicControlBus, RCSettingsLocal->GetMusicVolume());
					const FSoundControlBusMixStage SoundFXControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, SoundFXControlBus, RCSettingsLocal->GetSoundFXVolume());
					const FSoundControlBusMixStage DialogueControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, DialogueControlBus, RCSettingsLocal->GetDialogueVolume());
					const FSoundControlBusMixStage VoiceChatControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, VoiceChatControlBus, RCSettingsLocal->GetVoiceChatVolume());
			...
			ApplyDynamicRangeEffectsChains(RCSettingsLocal->IsHDRAudioModeEnabled());
		}
	}
}

void URCAudioMixEffectsSubsystem::ApplyDynamicRangeEffectsChains(bool bHDRAudio)
{
	TArray<FRCAudioSubmixEffectsChain> AudioSubmixEffectsChainToApply;
	TArray<FRCAudioSubmixEffectsChain> AudioSubmixEffectsChainToClear;
	...
	for (const FRCAudioSubmixEffectsChain& EffectChainToClear : AudioSubmixEffectsChainToClear)
	{
		bool bAddToList = true;

		for (const FRCAudioSubmixEffectsChain& SubmixEffectChain : AudioSubmixEffectsChainToApply)
		{
...
void URCAudioMixEffectsSubsystem::OnLoadingScreenStatusChanged(bool bShowingLoadingScreen)
{
	ApplyOrRemoveLoadingScreenMix(bShowingLoadingScreen);
}

void URCAudioMixEffectsSubsystem::ApplyOrRemoveLoadingScreenMix(bool bWantsLoadingScreenMix)
{
...
bool URCAudioMixEffectsSubsystem::DoesSupportWorldType(const EWorldType::Type World) const
{

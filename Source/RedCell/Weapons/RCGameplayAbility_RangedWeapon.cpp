...
#include "Weapons/RCGameplayAbility_RangedWeapon.h"
#include "Weapons/RCRangedWeaponInstance.h"
#include "Physics/RCCollisionChannels.h"
#include "RCLogChannels.h"
#include "AIController.h"
#include "NativeGameplayTags.h"
#include "Weapons/RCWeaponStateComponent.h"
#include "AbilitySystemComponent.h"
#include "AbilitySystem/RCGameplayAbilityTargetData_SingleTargetHit.h"
#include "DrawDebugHelpers.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(RCGameplayAbility_RangedWeapon)

namespace RCConsoleVariables
{
	static float DrawBulletTracesDuration = 0.0f;
	static FAutoConsoleVariableRef CVarDrawBulletTraceDuraton(
		TEXT("RedCell.Weapon.DrawBulletTraceDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHits(
		TEXT("RedCell.Weapon.DrawBulletHitDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHitRadius(
		TEXT("RedCell.Weapon.DrawBulletHitRadius"),
		DrawBulletHitRadius,
...
URCGameplayAbility_RangedWeapon::URCGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	SourceBlockedTags.AddTag(TAG_WeaponFireBlocked);
}

URCRangedWeaponInstance* URCGameplayAbility_RangedWeapon::GetWeaponInstance() const
{
	return Cast<URCRangedWeaponInstance>(GetAssociatedEquipment());
}

bool URCGameplayAbility_RangedWeapon::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	if (bResult)
	{
		...
		{
			UE_LOG(LogRCAbilitySystem, Error, TEXT("Weapon ability %s cannot be activated because there is no associated ranged weapon (equipment instance=%s but needs to be derived from %s)"),
				*GetPathName(),
				*GetPathNameSafe(GetAssociatedEquipment()),
				*URCRangedWeaponInstance::StaticClass()->GetName());
			bResult = false;
		}
	}
...
int32 URCGameplayAbility_RangedWeapon::FindFirstPawnHitResult(const TArray<FHitResult>& HitResults)
{
...
void URCGameplayAbility_RangedWeapon::AddAdditionalTraceIgnoreActors(FCollisionQueryParams& TraceParams) const
{
...
ECollisionChannel URCGameplayAbility_RangedWeapon::DetermineTraceChannel(FCollisionQueryParams& TraceParams, bool bIsSimulated) const
{
	return RC_TraceChannel_Weapon;
}

FHitResult URCGameplayAbility_RangedWeapon::WeaponTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHitResults) const
{
...
FVector URCGameplayAbility_RangedWeapon::GetWeaponTargetingSourceLocation() const
{
...
FTransform URCGameplayAbility_RangedWeapon::GetTargetingTransform(APawn* SourcePawn, ERCAbilityTargetingSource Source) const
{
	check(SourcePawn);
	AController* SourcePawnController = SourcePawn->GetController(); 
	URCWeaponStateComponent* WeaponStateComponent = (SourcePawnController != nullptr) ? SourcePawnController->FindComponentByClass<URCWeaponStateComponent>() : nullptr;

	// The caller should determine the transform without calling this if the mode is custom!
	check(Source != ERCAbilityTargetingSource::Custom);
	...
	if ((Controller != nullptr) && ((Source == ERCAbilityTargetingSource::CameraTowardsFocus) || (Source == ERCAbilityTargetingSource::PawnTowardsFocus) || (Source == ERCAbilityTargetingSource::WeaponTowardsFocus)))
	{
		...
		if (Source == ERCAbilityTargetingSource::CameraTowardsFocus)
		{
	...
	if ((Source == ERCAbilityTargetingSource::WeaponForward) || (Source == ERCAbilityTargetingSource::WeaponTowardsFocus))
	{
		SourceLoc = GetWeaponTargetingSourceLocation();
	}
	else
	...
	if (bFoundFocus && ((Source == ERCAbilityTargetingSource::PawnTowardsFocus) || (Source == ERCAbilityTargetingSource::WeaponTowardsFocus)))
	{
...
FHitResult URCGameplayAbility_RangedWeapon::DoSingleBulletTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHits) const
{
#if ENABLE_DRAW_DEBUG
	if (RCConsoleVariables::DrawBulletTracesDuration > 0.0f)
	{
		static float DebugThickness = 1.0f;
		DrawDebugLine(GetWorld(), StartTrace, EndTrace, FColor::Red, false, RCConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
	}
...
void URCGameplayAbility_RangedWeapon::PerformLocalTargeting(OUT TArray<FHitResult>& OutHits)
{
	APawn* const AvatarPawn = Cast<APawn>(GetAvatarActorFromActorInfo());

	URCRangedWeaponInstance* WeaponData = GetWeaponInstance();
	if (AvatarPawn && AvatarPawn->IsLocallyControlled() && WeaponData)
	{
		...
		const FTransform TargetTransform = GetTargetingTransform(AvatarPawn, ERCAbilityTargetingSource::CameraTowardsFocus);
		...
		if (RCConsoleVariables::DrawBulletTracesDuration > 0.0f)
		{
			...
			DrawDebugLine(GetWorld(), InputData.StartTrace, InputData.StartTrace + (InputData.AimDir * 100.0f), FColor::Yellow, false, RCConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
		}
#endif
...
void URCGameplayAbility_RangedWeapon::TraceBulletsInCartridge(const FRangedWeaponFiringInput& InputData, OUT TArray<FHitResult>& OutHits)
{
	URCRangedWeaponInstance* WeaponData = InputData.WeaponData;
	check(WeaponData);
	...
	for (int32 BulletIndex = 0; BulletIndex < BulletsPerCartridge; ++BulletIndex)
	{
		...
		{
#if ENABLE_DRAW_DEBUG
			if (RCConsoleVariables::DrawBulletHitDuration > 0.0f)
			{
				DrawDebugPoint(GetWorld(), Impact.ImpactPoint, RCConsoleVariables::DrawBulletHitRadius, FColor::Red, false, RCConsoleVariables::DrawBulletHitRadius);
			}
#endif
...
void URCGameplayAbility_RangedWeapon::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
...
void URCGameplayAbility_RangedWeapon::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled)
{
...
void URCGameplayAbility_RangedWeapon::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& InData, FGameplayTag ApplicationTag)
{
	UAbilitySystemComponent* MyAbilityComponent = CurrentActorInfo->AbilitySystemComponent.Get();
	check(MyAbilityComponent);
	...
	if (const FGameplayAbilitySpec* AbilitySpec = MyAbilityComponent->FindAbilitySpecFromHandle(CurrentSpecHandle))
	{
		...
		if (!bProjectileWeapon)
		{
			if (AController* Controller = GetControllerFromActorInfo())
			{
				...
				{
					// Confirm hit markers
					if (URCWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<URCWeaponStateComponent>())
					{
		...
		{
			UE_LOG(LogRCAbilitySystem, Warning, TEXT("Weapon ability %s failed to commit (bIsTargetDataValid=%d)"), *GetPathName(), bIsTargetDataValid ? 1 : 0);
			K2_EndAbility();
		}
	}
...
void URCGameplayAbility_RangedWeapon::StartRangedWeaponTargeting()
{
	...
	check(Controller);
	URCWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<URCWeaponStateComponent>();
	...
	if (FoundHits.Num() > 0)
	{
		...
		{
			FRCGameplayAbilityTargetData_SingleTargetHit* NewTargetData = new FRCGameplayAbilityTargetData_SingleTargetHit();

...
#include "RCGameplayAbility_RangedWeapon.generated.h"

enum ECollisionChannel : int;

class APawn;
class URCRangedWeaponInstance;
class UObject;
...
UENUM(BlueprintType)
enum class ERCAbilityTargetingSource : uint8
{
 ...
 */
UCLASS()
class URCGameplayAbility_RangedWeapon : public URCGameplayAbility_FromEquipment
{
	GENERATED_BODY()

public:

	URCGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	UFUNCTION(BlueprintCallable, Category="RedCell|Ability")
	URCRangedWeaponInstance* GetWeaponInstance() const;
...
protected:
	struct FRangedWeaponFiringInput
	{
		...
		URCRangedWeaponInstance* WeaponData = nullptr;
...
protected:
	...
	FTransform GetTargetingTransform(APawn* SourcePawn, ERCAbilityTargetingSource Source) const;

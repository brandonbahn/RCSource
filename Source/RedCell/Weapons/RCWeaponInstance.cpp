...
#include UE_INLINE_GENERATED_CPP_BY_NAME(RCWeaponInstance)

class UAnimInstance;
struct FGameplayTagContainer;

URCWeaponInstance::URCWeaponInstance(const FObjectInitializer& ObjectInitializer)
    : Super(ObjectInitializer)
{
    ...
    if (APawn* Pawn = GetPawn())
    {
        ...
        {
            if (URCHealthComponent* HealthComponent = URCHealthComponent::FindHealthComponent(GetPawn()))
            {
...
void URCWeaponInstance::OnEquipped()
{
    Super::OnEquipped();
...
void URCWeaponInstance::OnUnequipped()
{
    Super::OnUnequipped();
...
void URCWeaponInstance::UpdateFiringTime()
{
    UWorld* World = GetWorld();
    check(World);
...
float URCWeaponInstance::GetTimeSinceLastInteractedWith() const
{
    UWorld* World = GetWorld();
    check(World);
...
TSubclassOf<UAnimInstance> URCWeaponInstance::PickBestAnimLayer(bool bEquipped, const FGameplayTagContainer& CosmeticTags) const
{
    const FRCAnimLayerSelectionSet& SetToQuery = (bEquipped ? EquippedAnimSet : UnequippedAnimSet);
    return SetToQuery.SelectBestLayer(CosmeticTags);
}

const FPlatformUserId URCWeaponInstance::GetOwningUserId() const
{
...
void URCWeaponInstance::ApplyDeviceProperties()
{
...
void URCWeaponInstance::RemoveDeviceProperties()
{
    const FPlatformUserId UserId = GetOwningUserId();
...
void URCWeaponInstance::OnDeathStarted(AActor* OwningActor)
{
